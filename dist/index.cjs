...
__export(index_exports, {
  BankABI: () => Bank_default,
  MyTokenABI: () => MyToken_default,
  NETWORKS: () => NETWORKS,
  formatAddress: () => formatAddress,
  getBankBalance: () => getBankBalance,
  getBankContract: () => getBankContract,
  useBankApp: () => useBankApp,
  useMyHook: () => useMyHook,
  useWallet: () => useWallet
});
...
async function getBankContract() {
  ...
  if (!import_ethers.ethers.utils.isAddress(address)) {
    throw new Error("Invalid bank contract address");
  }
  const abi = Bank_default.abi;
  return new import_ethers.ethers.Contract(address, abi, signer);
}
...
async function getBankBalance(address) {
  ...
  return import_ethers2.ethers.utils.formatEther(bal);
}
...
function useWallet() {
  ...
  const getProvider = (0, import_react2.useCallback)(() => {
    ...
    return new import_ethers3.ethers.providers.Web3Provider(ethereum);
  }, [getEthereum]);
  const refreshBalance = (0, import_react2.useCallback)(async () => {
    if (!account) return;
    const provider = getProvider();
    const bal = await provider.getBalance(account);
    setBalance(import_ethers3.ethers.utils.formatEther(bal));
  }, [account, getProvider]);
  const connect = (0, import_react2.useCallback)(async () => {
    try {
      ...
      const ethereum = getEthereum();
      if (!ethereum) throw new Error("No wallet found");
      const provider = new import_ethers3.ethers.providers.Web3Provider(ethereum);
...
function useBankApp() {
  ...
  const refreshBalance = (0, import_react3.useCallback)(async () => {
    try {
      ...
      setBalance(import_ethers4.ethers.utils.formatEther(wei));
    } catch (e) {
      console.error(e);
  ...
  const deposit = (0, import_react3.useCallback)(
    async (amountEth) => {
      try {
        ...
        const value = import_ethers4.ethers.utils.parseEther(amountEth);
